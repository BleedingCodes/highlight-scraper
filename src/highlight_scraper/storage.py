...
0.4.0 additions:
  - delete_capture(row_id)  — per-row delete
  - search(query, ...)      — full-text search across capture bodies
"""
...
class CaptureStore:
    ...
    def update_text_for_last(self, text: str, session: str = None):
        ...
        that's just being extended or shrunk into the SAME row instead of
        inserting a new one per pixel of drag.
        """
        ts = datetime.now().isoformat(timespec="seconds")
        with self._lock:
            if session:
    ...
    # ── 0.4.0: per-row delete ────────────────────────────────────────────────

    def delete_capture(self, row_id: int) -> bool:
        """
        Permanently delete one capture by its id.
        Returns True if a row was deleted, False if the id didn't exist.
        Thread-safe; safe to call while capture is running.
        """
        with self._lock:
            cur = self.conn.execute(
                "DELETE FROM captures WHERE id=?", (row_id,)
            )
            self.conn.commit()
        return cur.rowcount > 0

    # ── 0.4.0: full-text search ──────────────────────────────────────────────

    def search(self, query: str, session: str = None, limit: int = 200) -> list:
        """
        Case-insensitive substring search across capture text bodies.

        Args:
            query:   The string to search for.
            session: Restrict to one session name, or None for all sessions.
            limit:   Maximum rows to return (most recent first).

        Returns a list of Capture dataclass instances, most recent first.
        """
        sql = (
            "SELECT id, text, captured_at, session, source_window, source_app, tag "
            "FROM captures WHERE text LIKE ?"
        )
        params: list = [f"%{query}%"]
        if session:
            sql += " AND session=?"
    ...
    def query(self, session: str = None, limit: int = 200) -> list:
